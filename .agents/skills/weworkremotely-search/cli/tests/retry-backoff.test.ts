...
// immediately so the exhaustion case does not sleep through the real 500ms -> 8s
// backoff schedule.
...
describe("htmlFetch retry/backoff", () => {
  ...
  test("returns the documented empty string on 404 without retrying", async () => {
    ...
    const html = await htmlFetch("https://weworkremotely.com/remote-jobs/x");
    expect(html).toBe("");
  ...
  test("gives up after the initial attempt plus six retries on persistent 5xx", async () => {
    ...
    await expect(htmlFetch("https://weworkremotely.com/remote-jobs/x")).rejects.toThrow(/500/);
